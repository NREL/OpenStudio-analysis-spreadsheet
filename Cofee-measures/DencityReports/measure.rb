...
class DencityReports < OpenStudio::Ruleset::ReportingUserScript
  ...
  def run(runner, user_arguments)
    ...
    begin
            ...
            meter_strings.each do |meter_string|
              ...
              unless sqlFile.timeSeries("RUN PERIOD 1", "Zone Timestep", meter_string, "").empty?
                ...
                if ts_values.size == ts_times.size
                  timeseries_out = Hash.new
                  for i in 0..ts_times.size - 1
                    timeseries_out[Time.parse(ts_times[i].toString()).to_i*1000] = ts_values[i]
                  end
                  title = meter_string.split(':')
                  title = title[0]
                  file.puts "{"
                  file.puts "Fuel: #{title}"
                  file.puts "Interval (s): #{Time.parse(ts_times[1].toString()).to_i - Time.parse(ts_times[0].toString()).to_i}"
                  file.puts "Data:"
                  file.puts(JSON::pretty_generate(timeseries_out))
                  file.puts "},"
                else
      ...
      printer.print(DENCITYPROFILE, :min_percent => 1)
