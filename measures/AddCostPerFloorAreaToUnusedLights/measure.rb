...
#start the measure
class AddCostPerFloorAreaToUnusedLights < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "AddCostPerFloorAreaToUnusedLights"
  end
  ...
  def arguments(model)
    ...
    lights_def_args_hash.sort.map do |key,value|
      ...
      if not value.quantity > 0 and not value.wattsperSpaceFloorArea.empty?
