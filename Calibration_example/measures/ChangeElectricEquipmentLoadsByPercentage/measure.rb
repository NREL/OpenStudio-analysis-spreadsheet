...
class ChangeElectricEquipmentLoadsByPercentage < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    elecequip_power_change_percent.setDisplayName("Electric Equipment Power change (%).")
  ...
  def run(model, runner, user_arguments)
    ...
    def alter_performance_and_lcc(object, elecequip_power_change_percent, material_and_installation_cost, demolition_cost, om_cost, years_until_costs_start, expected_life, om_frequency, runner)
    ...
    space_types.each do |space_type|
      ...
      space_type_equipments.each do |space_type_equipment|
        ...
        if not cloned_elecequip_defs[exist_def.name.get.to_s].nil?
          ...
          new_def_name = new_def.setName("#{exist_def.name.get} + #{elecequip_power_change_percent.round(2)} % change")
          ...
          alter_performance_and_lcc(new_def, elecequip_power_change_percent, material_and_installation_cost, demolition_cost, om_cost, years_until_costs_start, expected_life, om_frequency, runner)
        ...
        updated_instance_name = space_type_equipment.setName("#{space_type_equipment.name} #{elecequip_power_change_percent.round(2)} % change")
    ...
    spaces.each do |space|
      ...
      space_equipments.each do |space_equipment|
        ...
        if cloned_elecequip_defs.any? {|k,v| k.include? exist_def.name.get.to_s}
          ...
          #clone rename and add to hash
          new_def = exist_def.clone(model)
          new_def_name = new_def.setName("#{new_def.name} + #{elecequip_power_change_percent.round(2)} % change")
        ...
        updated_instance_name = space_equipment.setName("#{space_equipment.name} #{elecequip_power_change_percent.round(2)} % change")
