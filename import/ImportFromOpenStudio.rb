...
# == Usage
#
#   ruby ImportFromOpenStudio.rb ./path/to/project/directory
#
# == Examples
#
#   ruby ImportFromOpenStudio.rb C:/working/MyOpenStudioProjects/20140210_TestProject
#
######################################################################
    ...
    if not ivar.to_MeasureGroup.empty?
      ...
      ivar.to_MeasureGroup.get.measures(false).each { |measure|
        if not measure.to_RubyMeasure.empty?
          ...
          if last_measure.nil? or not (last_measure == rm)
            # keep this measure
            last_measure = rm
