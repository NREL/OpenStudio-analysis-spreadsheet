...
class ImproveFanBeltEfficiency < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    motor_eff.setDisplayName("Motor Efficiency Change(%).")
    motor_eff.setDefaultValue(0.0)
    args << motor_eff

    fan_eff = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("fan_eff",true)
    fan_eff.setDisplayName("Fan Efficiency Change(%).")
    fan_eff.setDefaultValue(0.0)
    args << fan_eff
  ...
  def run(model, runner, user_arguments)
    ...
    missing_initial_fan_efficiency = 0
    ...
    loops.each do |loop|
      ...
      supply_components.each do |supply_component|
        ...
        if not hVACComponent.empty?
          ...
          target_fan_efficiency = initial_fan_efficiency + fan_eff * 0.01
          initial_fan_efficiency_values << initial_fan_efficiency
          if target_fan_efficiency > 1
            hVACComponent.setFanEfficiency(1.0)
            runner.registerWarning("Requested efficiency of #{target_fan_efficiency*100}% for #{hVACComponent.name} is not possible. Setting motor efficiency to 100%.")
          elsif target_fan_efficiency < 0
            hVACComponent.setFanEfficiency(0.0)
            runner.registerWarning("Requested efficiency of #{target_fan_efficiency*100}% for #{hVACComponent.name} is not possible. Setting motor efficiency to 0%.")
          else
            hVACComponent.setFanEfficiency(target_fan_efficiency)
            runner.registerInfo("Changing the motor efficiency from #{initial_fan_efficiency*100}% to #{target_fan_efficiency*100}% for '#{hVACComponent.name}' onloop '#{loop.name}.'")
          end
