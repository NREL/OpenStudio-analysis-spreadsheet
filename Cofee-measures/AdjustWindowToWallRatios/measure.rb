...
class SetWindowToWallRatioByFacade < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    wwr_multiplier = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("wwr_multiplier",true)
    wwr_multiplier.setDisplayName("Window to Wall Ratio Multiplier.")
    wwr_multiplier.setDefaultValue(1.0)
    args << wwr_multiplier
  ...
  def run(model, runner, user_arguments)
    ...
    wwr_multiplier = runner.getDoubleArgumentValue("wwr_multiplier",user_arguments)
    sillHeight = runner.getDoubleArgumentValue("sillHeight",user_arguments)


    #check reasonableness of fraction
    if wwr_multiplier < 0 
      runner.registerError("Window to Wall Ratio Multiplier must be greater than or equal to 0.")
      return false
    end
    #check reasonableness of sillHeight
    if sillHeight <= 0
    ...
    runner.registerInitialCondition("The model's initial window to wall ratio for exterior walls was #{starting_wwr}.")
    ...
    runner.registerFinalCondition("The model's final window to wall ratio for exterior walls is #{final_wwr}. Window area increased by #{neat_numbers(increase_window_area_ip.value,0)} (ft^2). The material and construction costs increased by $#{neat_numbers(envelope_cost,0)}.")
