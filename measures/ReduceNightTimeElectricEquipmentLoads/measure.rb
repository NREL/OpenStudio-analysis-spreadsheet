#start the measure
class ReduceNightTimeElectricEquipmentLoads < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    elec_load_def = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("elec_load_def", elec_load_def_handles, elec_load_def_display_names)
  ...
  def run(model, runner, user_arguments)
    ...
    if elec_load_def.empty?
      ...
      if not elec_load_def.get.to_ElectricEquipmentDefinition.empty?
    ...
    equip_sch_names.uniq.each do |equip_sch_name|
      ...
      if not equip_sch.to_ScheduleRuleset.empty?
      ...
      end #if not equip_sch.to_ScheduleRuleset.empty?
    end #end of equip_sch_names.uniq.each do
    ...
    if equip_sch_names.uniq.size == 0
