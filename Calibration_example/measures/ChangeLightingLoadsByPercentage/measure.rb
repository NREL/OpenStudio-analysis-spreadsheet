#start the measure
class ChangeLightingLoadsByPercentage < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see
  def name
    return "Change Lighting Loads by Percentage"
  end
  ...
  def arguments(model)
    ...
    #make an argument for change percentage
    lighting_power_change_percent = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("lighting_power_change_percent",true)
    lighting_power_change_percent.setDisplayName("Lighting Power change (%).")
    lighting_power_change_percent.setDefaultValue(30.0)
    args << lighting_power_change_percent
  ...
  def run(model, runner, user_arguments)
    ...
    lighting_power_change_percent = runner.getDoubleArgumentValue("lighting_power_change_percent",user_arguments)
    ...
    building_LPD = nil
    begin
      building_LPD = building.lightingPowerPerFloorArea
      rescue
    end

    if not building_LPD.nil?
      ...
      # can't calculate LPD, building may not have surfaces
      runner.registerInitialCondition("The model's initial building lighting power was  #{neat_numbers(building_lighting_power,0)} watts.")
    end
    ...
    def alter_performance_and_lcc(object, lighting_power_change_percent, material_and_installation_cost, demolition_cost, om_cost, years_until_costs_start, expected_life, om_frequency, runner)

      #edit clone based on percentage change
      new_def = object
      if not new_def.lightingLevel.empty?
        new_lighting_level = new_def.setLightingLevel(new_def.lightingLevel.get + new_def.lightingLevel.get*lighting_power_change_percent*0.01)
      elsif not new_def.wattsperSpaceFloorArea.empty?
        new_lighting_per_area = new_def.setWattsperSpaceFloorArea(new_def.wattsperSpaceFloorArea.get + new_def.wattsperSpaceFloorArea.get*lighting_power_change_percent*0.01)
      elsif not new_def.wattsperPerson.empty?
        new_lighting_per_person = new_def.setWattsperPerson(new_def.wattsperPerson.get + new_def.wattsperPerson.get*lighting_power_change_percent*0.01)
      else
    ...
    space_types.each do |space_type|
      ...
      space_type_lights.each do |space_type_light|
        ...
        if not cloned_lights_defs[exist_def.name.get.to_s].nil?
          ...
          new_def_name = new_def.setName("#{exist_def.name.get} + #{lighting_power_change_percent.round(2)} % change")
          ...
          alter_performance_and_lcc(new_def, lighting_power_change_percent, material_and_installation_cost, demolition_cost, om_cost, years_until_costs_start, expected_life, om_frequency, runner)
        ...
        updated_instance_name = space_type_light.setName("#{space_type_light.name} #{lighting_power_change_percent.round(2)} % change")
      ...
      space_type_luminaires.each do |space_type_luminaire|
        ...
        if not cloned_luminaire_defs[exist_def.name.get.to_s].nil?
          new_def = cloned_luminaire_defs[exist_def.name.get]
        else
          #clone rename and add to hash
          new_def = exist_def.clone(model)
          new_def_name = new_def.setName("#{new_def.name} + #{lighting_power_change_percent.round(2)} % change")
        ...
        #link instance with clone and rename
        updated_instance = space_type_luminaire.setLuminaireDefinition(new_def.to_LuminaireDefinition.get)
        updated_instance_name = space_type_luminaire.setName("#{space_type_luminaire.name} #{lighting_power_change_percent.round(2)} % change")
    ...
    spaces.each do |space|
      ...
      space_lights.each do |space_light|
        ...
        if not cloned_lights_defs[exist_def.name.get.to_s].nil?
          ...
          #clone rename and add to hash
          new_def = exist_def.clone(model)
          new_def_name = new_def.setName("#{new_def.name} + #{lighting_power_change_percent} percent change")
        ...
        updated_instance_name = space_light.setName("#{space_light.name} #{lighting_power_change_percent} percent change")
      ...
      space_luminaires.each do |space_luminaire|
        ...
        #link instance with clone and rename
        updated_instance = space_luminaire.setLuminaireDefinition(new_def.to_LuminaireDefinition.get)
        updated_instance_name = space_luminaire.setName("#{space_light.name} + #{lighting_power_change_percent} percent change")
    ...
    if not building_LPD.nil?
      ...
      runner.registerFinalCondition("The model's final building lighting power was  #{neat_numbers(final_building_lighting_power,0)} watts. Initial capital costs associated with the improvements are $#{neat_numbers(yr0_capital_totalCosts,0)}.")
    end
...
ChangeLightingLoadsByPercentage.new.registerWithApplication